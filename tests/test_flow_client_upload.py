...
class GeminiOmniModelRegistryTests(unittest.TestCase):
    """验证 Gemini Omni Flash (abra / Omni 1.1) 模型注册项的字段一致性。
    ...
    def test_model_key_matches_duration_suffix(self):
        ...
        expected_keys = {
            ...
            "r2v": "abra_r2v_{d}s",
            "i2v": "abra_i2v_{d}s",
            "fl": "omni_flash_i2v_{d}s_first_last",
        }
            ...
            for kind, key_tpl in expected_keys.items():
                ...
                for tail in ("", "_1080p", "_4k", "_360p"):
                    ...
                    for orientation, aspect in (
                        ...
                        self.assertEqual(cfg["model_key"], upstream_key, name)
                        self.assertEqual(cfg["aspect_ratio"], aspect, name)

    def test_upsample_variants_use_veo_upsamplers(self):
        """1080p/4K 变体上采样必须复用 veo upsampler（projectInitialData 抓包验证）。"""
        for name, cfg in self.cfg.items():
            if not name.startswith("gemini_omni_"):
                continue
            if name.endswith("_1080p"):
                up = cfg.get("upsample")
                self.assertIsNotNone(up, f"{name} missing upsample")
                self.assertEqual(up["resolution"], "VIDEO_RESOLUTION_1080P", name)
                self.assertEqual(up["model_key"], "veo_3_1_upsampler_1080p", name)
            elif name.endswith("_4k"):
                up = cfg.get("upsample")
                self.assertIsNotNone(up, f"{name} missing upsample")
                self.assertEqual(up["resolution"], "VIDEO_RESOLUTION_4K", name)
                self.assertEqual(up["model_key"], "veo_3_1_upsampler_4k", name)
            else:
                self.assertNotIn("upsample", cfg, f"{name} should not have upsample")

    def test_i2v_entries_require_single_start_frame(self):
        """Omni i2v 仅首帧：恰好 1 张图（上游 requirement 只有 START_IMAGE）。"""
        for name, cfg in self.cfg.items():
            if not name.startswith("gemini_omni_i2v_"):
                continue
            self.assertEqual(cfg["video_type"], "i2v", name)
            self.assertTrue(cfg["supports_images"], name)
            self.assertEqual(cfg["min_images"], 1, name)
            self.assertEqual(cfg["max_images"], 1, name)
            self.assertTrue(cfg.get("use_v2_model_config"), name)

    def test_fl_entries_require_exactly_two_frames(self):
        """Omni 首尾帧：恰好 2 张图（上游强制 START_IMAGE+END_IMAGE，不接受单帧）。"""
        for name, cfg in self.cfg.items():
            if not name.startswith("gemini_omni_fl_"):
                continue
            # fl 走 handler 的 i2v 分支（start/end image 端点），靠图片数量区分
            self.assertEqual(cfg["video_type"], "i2v", name)
            self.assertTrue(cfg["supports_images"], name)
